...
计算生成以0为开头和以1为开头的alternative string需要的次数。用两个char分别代表以0开头和以1开头的交替字符串当前index上的字符，两个int记录需要改变的次数。遍历当前字符串，比较当前index上的字符和交替字符串，如果不同则说明需要改变，int +1。然后修改交替字符串的char，如果是'0'改成'1'，'1'改成'0'。

Time complexity: $O(n)$, space complexity: $ O(1) $

可以优化的地方：如何修改交替字符串的char？

010101 -> 除2余数

**Improve**: 判断当前字符是否需要改变，成为 0101 字符串的判断条件 为 char - '0' == index % 2

#### 1759 Count Number of Homogenous Substrings

**My solution**:

这道题求字符串中同构子字符串的个数。因为子字符串要求连续，提示思考前后字符与同构子字符串的关系。如果前后字符相同，e.g. aa, 那么第二个a多构成了2个同构子串。如果前后字符不同，e.g. ab, b构成了1个同构子串。

用一个int类型的变量 temp记录每个字符构成了几个同构子串。如果前后字符相同，temp 加1。如果不同，则temp为1。

思路错误：什么时候更新总count的个数？应该在循环开始还是循环结束？如果是循环开始，则更新的是上一位的个数，最后一位的count需要另计。如果是循环结束，则最开始的个数需要另计。

代码错误：java里不能用int a = 10 ^ 7 表示幂, 而是 ```Math.pow()```
...
二分法查找包中球的个数，也就是penalty

假设 penalty 为3，也就是袋中球的个数至多为3. 怎么去拆分包呢？
...
拆分一个包所需要的移动次数为 `(a - 1) / penlaty`. 拆分所有包的个数就是 拆分每一个包的个数的和。

如果给定的operation > 当前operation, 说明penalty 太大, right = middle。

如果给定的operation < 当前operation, 说明penalty 太小，left = middle + 1。

思路错误：一般二分法里，缩小查询范围都是 `right = mid - 1` 或者 `left = mid + 1`。但这道题不同的是，如果给定的oper < 当前oper, 说明挪动次数超过限制，必须要缩小到middle右边。mid 不满足情况。而当给定的oper > 当前 oper，说明挪动次数没有超过限制，mid是可行的，但并不确定为最优解，需要缩小right到mid(包括mid)。
