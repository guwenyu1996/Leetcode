...
SYN: 在连接建立时用来同步序号。在连接建立时用来同步序号。当SYN=1，ACK=0，表明是连接请求报文，若同意连接，则响应报文中应该使SYN=1，ACK=1
...
- A 发送连接关闭请求，FIN = 1，seq=u
- B 收到之后发出确认，回复ACK，关闭连接。ACKnum=u+1, seq=v
- A 收到后发出确认，回复ACK, 进入 TIME-WAIT 状态，等待 2 MSL（最大报文存活时间）后释放连接。在等待过程中，如果收到 FIN, 会重复发送ACK
- B 收到ACK, 连接关闭